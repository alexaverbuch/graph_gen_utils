...
public class NeoFromFile {
   ...
   * values (as given by the {@link String} values in relTypes parameter) from
   * the Neo4j instance.
   * 
   * @param transNeo {@link GraphDatabaseService} representing a Neo4j instance
  ...
  /**
   * Deletes a specified number of {@link Relationship}s by selecting them from
   * among all {@link Relationship} s and then deleting them from the Neo4j
   * instance. Selection is performed uniformly random.
   * 
   * @param transNeo {@link GraphDatabaseService} representing a Neo4j instance
   * @param relsToDelete number of {@link Relationship} s to delete
   * @param maxRelId {@link Relationship} s to delete are chosen from the ID
   *          space between 0 and maxRelId
   */
  public static void removeRandomRelationships(GraphDatabaseService transNeo,
    long relsToDelete, long maxRelId) {
    
    long time = System.currentTimeMillis();
    System.out.printf("Deleting random relationships...");
    ...
    try {
      ...
      long relToDeleteId = -1;
      HashSet<Long> alreadyDeletedRels = new HashSet<Long>();
      
      while (deletedRels < relsToDelete) {
        relToDeleteId = Math.round(rand.nextDouble() * maxRelId);
        
        if (alreadyDeletedRels.add(relToDeleteId) == false)
          continue;
        
        Relationship relToDelete = null;
        try {
          relToDelete = transNeo.getRelationshipById(relToDeleteId);
        } catch (Exception e) {
